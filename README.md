...
- `skills/stock-return-reconciliation/scripts/build_demo_workbook.py`: 검토용
  XLSX workbook 생성 스크립트
...
- `skills/skill-quality-review-ko/SKILL.md`: 한국어 Codex Skill 초안을 제출 전
  재현성, 안전성, 검증 가능성 기준으로 평가하는 보조 Skill
- `skills/skill-quality-review-ko/references/`: 입력 계약, 루브릭, 안전 가드레일,
  출력 템플릿, 깨뜨려 보는 질문, 예시 스니펫
- `skills/skill-quality-review-ko/scripts/validate_skill_review_pack.py`: 보조 Skill
  구조 검증 스크립트
- `outputs/skill-quality-review-sample-review.md`: 보조 Skill 샘플 리뷰 결과
...
저장소 루트에서 실행합니다. Python 3 표준 라이브러리만 사용하며, 외부 패키지나
네트워크 접근은 필요하지 않습니다.
...
- 검증 스크립트가 누락 파일, 스키마 오류, 비밀값 패턴, 거래 판단 경계 위반이
  없다고 보고합니다.
- 보조 Skill 검증 스크립트가 `skill-quality-review-ko`의 frontmatter, reference
  링크, 필수 섹션, 루브릭/안전/출력 템플릿 구성을 확인합니다.
...
- `python3 skills/stock-return-reconciliation/scripts/validate_sample.py`:
  `Validation passed. Checked 22 files, CSV schemas, numeric/date fields, outputs, workbook, secrets, and advice boundaries.`
- `python3 skills/skill-quality-review-ko/scripts/validate_skill_review_pack.py`:
  `Skill review pack validation passed: .../skills/skill-quality-review-ko`

## 보조 Skill: Skill 품질 평가하기

`skills/skill-quality-review-ko/`는 한국어 Codex Skill 초안을 공유하거나 제출하기
전에 재현성, 안전성, 검증 가능성을 점검하는 보조 Skill입니다. 기본 동작은 전체
재작성 대신 입력 계약, workflow, guardrails, 검증 루프, 깨뜨려 보는 질문을
평가하고 작은 수정 단위의 개선안을 제안하는 것입니다.
...
샘플 리뷰 결과는 `outputs/skill-quality-review-sample-review.md`에서 확인할 수
있습니다.
...
- `outputs/reconciliation-summary.json`
- `outputs/stock_return_reconciliation_demo.xlsx`
- `skills/skill-quality-review-ko/SKILL.md`
- `skills/skill-quality-review-ko/references/`
- `outputs/skill-quality-review-sample-review.md`
- 위 검증 명령
